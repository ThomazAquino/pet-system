...
const reduce = createReducer(
  initialState,
  on(TutorsActions.addTutor,
    (state, action) => tutorAdapter.addOne(action.tutor, state)
  ),
  on(TutorsActions.upsertTutor,
    (state, action) => tutorAdapter.upsertOne(action.tutor, state)
  ),
  on(TutorsActions.addTutors,
    (state, action) => tutorAdapter.addMany(action.tutors, state)
  ),
  on(TutorsActions.upsertTutors,
    (state, action) => tutorAdapter.upsertMany(action.tutors, state)
  ),
  on(TutorsActions.updateTutor,
    (state, action) => tutorAdapter.updateOne(action.tutor, state)
  ),
  on(TutorsActions.updateTutors,
    (state, action) => tutorAdapter.updateMany(action.tutors, state)
  ),
  on(TutorsActions.deleteTutor,
    (state, action) => tutorAdapter.removeOne(action.id, state)
  ),
  on(TutorsActions.deleteTutors,
    (state, action) => tutorAdapter.removeMany(action.ids, state)
  ),
  on(TutorsActions.loadTutors,
    (state, action) => tutorAdapter.addAll(action.tutors, state)
  ),
  on(TutorsActions.clearTutors,
    state => tutorAdapter.removeAll(state)
  ),
);
